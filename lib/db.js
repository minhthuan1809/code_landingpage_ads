...
function migrateDb() {
  ...
  const parentsForCname = prepare("SELECT id, domain, cname_target FROM parent_domains").all();
  for (const parent of parentsForCname) {
    if (!String(parent.cname_target || "").trim()) {
      prepare("UPDATE parent_domains SET cname_target = ? WHERE id = ?").run([
        `origin.${normalizeDomain(parent.domain)}`,
        parent.id,
      ]);
    }
  }
...
function resolveParentCnameTarget(row) {
  ...
  const raw = String(row?.cname_target || "").trim();
  if (!parent) return raw;
  if (!raw) return `origin.${parent}`;
  if (raw.includes(".")) return normalizeDomain(raw);
  return normalizeDomain(`${raw}.${parent}`);
}
...
function parseParentDomain(row, { includeSecrets = false } = {}) {
  ...
  const parsed = {
    ...
    cname_target: resolveParentCnameTarget(row),
  };
...
function getParentDomainCredentials(id) {
  ...
  return {
    ...
    cnameTarget: resolveParentCnameTarget(row),
  };
}
...
function resolveCfForDomain(domain) {
  ...
  return {
    ...
    cnameTarget: global.cname_target || null,
  };
}
...
function createParentDomain({
  ...
  const resolvedCname = resolveParentCnameTarget({
    domain: normalized,
    cname_target,
  });
  ...
  const created = getParentDomainById(row?.id);
  try {
    const { rememberPublicIp } = require("./server-ip");
    rememberPublicIp(server_ip);
  } catch {
    // ignore
  }
  return created;
...
function updateParentDomain(
  ...
  const nextCnameTarget =
    cname_target !== undefined
      ? resolveParentCnameTarget({ domain: normalized, cname_target })
      : existing.cname_target;
