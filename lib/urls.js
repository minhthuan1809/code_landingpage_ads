const { getPublicIpSync, hostMatchesMachine } = require("./server-ip");
...
function getServerConfig() {
  ...
  const { getLocalMachineIpv4s } = require("./server-ip");
  ...
  return {
    ...
    port: LANDING_PORT,
    publicIp,
    machineIps: getLocalMachineIpv4s(),
    baseUrl: landingUrl,
...
function isLocalAccessHost(host) {
  return hostMatchesMachine(host);
}
